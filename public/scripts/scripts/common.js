...
console.log('DaVinci 1.0');
let Harmony = null;

/*
window.onerror = function(msg, url, line, col, err) {
    var str = msg.toLowerCase();
    var sub = "script error";
    var dat = {
        call:    url,
        column:  col,
        line:    line,
    ...
    if (str.indexOf(sub) > -1){
        ...
        if(!dat.name){ dat.name = 'ScriptError'; }
        if(!dat.message){ dat.message = 'External script error'; }
    }
    console.log('ERROR:', dat);
    let inf = JSON.stringify(dat);
    fetch('/error', {method:'post', headers: {'content-type': 'application/json'}, body:inf})
    return false; // false.throw true.ignore
};

function errorHandler(ex){
    if(!navigator.onLine) { return; }
    let errorNames = ['EvalError', 'RangeError', 'ReferenceError', 'SyntaxError', 'TypeError', 'URIError'];
    let cal, col, lin, msg, mth, nam, stk, par, enm, spl, lst;
    nam = ex.name;
    msg = ex.message;
    stk = ex.stack || '';
    par = stk.split('\n');
    enm = par[0].split(':')[0];
    console.log('Error name', enm);
    if(errorNames.indexOf(enm) < 0){
        console.log('Error in Safari/Firefox');
        lst = par[0].trim();
        mth = lst.split('@')[0] || '';
        cal = lst.split('@')[1] || '';
        lin = cal.split(':').slice(-2)[0] || '';
        col = cal.split(':').slice(-2)[1] || '';
    } else {
        console.log('Error in Chrome/Brave');
        lst = par[1].trim();
        mth = lst.split('(')[0].substr(3).trim() || '';
        cal = lst.split('(')[1].split(')')[0] || lst;
        lin = cal.split(':').slice(-2)[0] || '';
        col = cal.split(':').slice(-2)[1] || '';
    }
    let dat = {
...
//const ROOT  = window.location.hostname=='localhost'?'':'/davinci';
...
function dayStart(sdate) {
    let date = new Date(sdate);
    let dini = new Date(date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate());
    return dini;
}
...
function getTime(sdate) {
    ...
    let h12  = (''+(h24>12?h24-12:h24)).padStart(2, '0');
    let mins = (''+date.getMinutes()).padStart(2, '0');
    let ampm = (h24>12?'pm':'am');
    return h12 + ':' + mins + ' ' + ampm;
}

function timeOrDate(sdate) {
    let now  = new Date();
    let dini = dayStart(now);
    let date = new Date(sdate);
    if(date<dini) {
        return (''+(date.getMonth()+1)).padStart(2, '0') + '-' + date.getDate();
    }
    return getTime(date);
}
...
function inputDate(date) {
    ...
    let res  = new Date(time.setMinutes(time.getMinutes()-time.getTimezoneOffset())).toJSON().substr(0,16);
    return res;
}

function UTCDate(date) {
    if(!date){ date = new Date().toJSON(); }
    let yy = parseInt(date.substr(0,4)  || '1999');
    let mo = parseInt(date.substr(5,2)  || '01')-1;
    let dd = parseInt(date.substr(8,2)  || '01');
    let hh = parseInt(date.substr(11,2) || '00');
    let mi = parseInt(date.substr(14,2) || '00');
    let ss = parseInt(date.substr(17,2) || '00');
    let ut = new Date(Date.UTC(yy,mo,dd,hh,mi,ss));
    console.log('UTC',ut,yy,mo,dd,hh,mi,ss)
    return ut;
}
...
function money(amount, decs=2) {
    return Number(parseFloat(amount).toFixed(decs)).toLocaleString(undefined, {useGrouping: true, minimumFractionDigits: decs, maximumFractionDigits: decs});
    //return Number(parseFloat(amount).toFixed(decs)).toLocaleString();
    //return parseFloat(amount).toFixed(decs);
}
...
function onResell(obj, item, order) {
    console.log('OnResell item', item.address);
    console.log('OnResell order', order.address);
    //window.event.stopPropagation();
...
async function sendPayment(receiver, amount, tokenId) {
    ...
    txn = Harmony.transactions.newTx({
        ...
        gasLimit: '100000000',  // gas limit, you can use string
...
async function signMessage(msg) {
    ...
    //let sig = null;
    try {    
        let res = await Davinci.wallet.sign(txn);
        //let res = await Davinci.wallet.signTransaction(txn);
        sig = res.signature;
        console.log('Messag', msg);
        console.log('Signed', res);
        console.log('String', sig.r + sig.s.substr(2) + sig.v.toString('16'));
    } catch(ex){
...
async function onSendAction(){
    ...
    try {
        ...
        // Cancel orders if any and amount = 1
        // TODO: 
        // - get orders by artwork=$1 and owner=$2
        // - 
    ...
    setSendStatus('SUCCESS, TOKEN SENT!');
    setSendAction('SEND AGAIN');
}
...
async function showVinciBalance(){
    if(!Davinci || !Davinci.address){ 
        console.log('No address for Vinci Balance');
        return;
    }
    console.log('Vinci Balance of', Davinci.address);

    let gas, ctr, res, bal;
    try {
        gas = { gasPrice: config.GASPRICE, gasLimit: config.GASLIMIT };
        ctr = await Davinci.contract(HRC20.abi, config.VinciToken);
        res = await ctr.methods.balanceOf(Davinci.address).call(gas);
        //console.log('Balance', res);
        bal = money(parseFloat(res.toString())/10**18);
        console.log('Vinci Balance', bal);
        $('account-vinci').innerHTML = bal;
    } catch(ex) {
        console.log('Error getting VINCI balance', ex);
        $('account-vinci').innerHTML = 'Error';
    }
}
...
async function resellBalance(obj, artwork, ownerid) {
    console.log('Balance', artwork, ownerid);
    obj.innerHTML = 'Wait...';
    let res, jsn;
    try {
...
async function removeOwner(obj, artwork, ownerid) {
    console.log('Remove owner', artwork, ownerid);
    let row = obj.parentNode;
    let tab = row.parentNode.parentNode;
    let res, jsn;
    try {
        tab.deleteRow(row.rowIndex);
        res = await fetch(`/api/removeowner/${artwork}/${ownerid}`);
        jsn = await res.json();
        console.log('Removed', jsn);
        //console.log('Removed', row.rowIndex);
    } catch(ex) {
        console.log('Error removing owner', ex.message);
    }
}
...
async function startDavinci() {
    ...
    try {    
        ...
        //console.log('Get Config', cfg);
        config.CHAINID   = cfg.chainid;
        config.NETURL    = cfg.neturl;
        config.NETWORK   = cfg.network;
        config.EXPLORER  = cfg.explorer;
        config.GATEWAY   = cfg.ipfsurl;
        config.GASPRICE  = cfg.gasprice;
        config.GASLIMIT  = cfg.gaslimit;
        config.GASBATCH  = cfg.gasbatch;
        config.GASORDER  = cfg.gasorder;
        config.GASDEPLOY = cfg.gasdeploy;
