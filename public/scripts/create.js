...
let session = {
	...
	media     : 'image', // text image audio music video movie book domain
...
function onSelectMedia(m) {
	...
	if(m=='image'){ $('cover').classList.add('hidden'); }
	else { $('cover').classList.remove('hidden'); $('file-preview').classList.add('hidden'); }
	switch(m){
		case 'image' : btn[0].classList.add('selected'); $('file-type').innerHTML = 'JPG, PNG, GIF'; break;
		case 'model' : btn[1].classList.add('selected'); $('file-type').innerHTML = 'GLB, GLTF'; break;
...
async function onFormSubmit() {
	...
	if(!Davinci.wallet){ setStatus('Wallet not connected', true); return; }
	...
	let inidate  = $('auction-ini').value;
	let enddate  = $('auction-end').value;
	let inidatex = new Date(inidate).toJSON();
	let enddatex = new Date(enddate).toJSON();
	...
	if(input.price>session.maxprice){
		setStatus('Max price for '+session.pricetag+' artists is '+session.maxprice+' ONE', true); 
		return;
	}
	...
	setStatus('Saving file, please wait...');
	...
	data.append('inidate',    input.inidate);
	data.append('enddate',    input.enddate);
...
async function main() {
	...
	$('auction-ini').value = inputDate(today);
	$('auction-end').value = inputDate(endday);
	startDavinci();
	calcPrice();
}
